...
LeftFrameTopY        EQU 100 ; set to 0 or else collision / end
...
LeftFrameLeftY       EQU 154 ; original value is 54
LeftFrameLeftFilehandle DW ?

LeftFrameRightWidth EQU  43
LeftFrameRightHeight EQU 426 ; original value is 426
LeftFrameRightFilename DB 'iceright.bin', 0
LeftFrameRightX      EQU 297
LeftFrameRightY      EQU 154 ; original value is 54
...
LeftFrameBottomY         EQU 552 ; original value is 451 | 546 is good
...
RightFrameTopY       EQU 100 ; Orig value 1
...
RightFrameBottomY        EQU 552 ; original value is 546
RightFrameBottomFilehandle DW ?

WideFrameWIDTH  EQU 250
WideFrameHEIGHT EQU 60 ; Original value is 60
WideFrameData   DB  WideFrameHEIGHT*WideFrameWIDTH DUP(0)

TallFrameWIDTH  EQU 60
TallFrameHEIGHT EQU 465 ; Original value is 465
...
;canChangePiece                  DB 1            
canChangePieceP1                DB 1                ; Shit that can change piece for every block
canInstantDropP1                DB 1                ; New flag for instant drop (replaces powerup5)
...
canChangePieceP2                DB 1                ; Shit that can change piece for every block
canInstantDropP2                DB 1                ; New flag for instant drop (replaces powerup5)
...
FRAMEHEIGHT         EQU  25     ;height of each frame in blocks | original value is 20 | 25 is good with curent
PLAYFIELDTOPY     EQU  4      ;gpt shis
...
leftPower6              DB  07h     ;6 key (instant drop - always available)
...
rightPower6             DB  0Ch     ;- key (instant drop - always available)
...
LeftScoreLocY       EQU 42
RightScoreLocX      EQU 87
RightScoreLocY      EQU 42 ; FUCKJANSEN
...
Menu11  DB "Please enter your username:" ;fucked with this added 'user"
M11sz   EQU 27
...
GE1sz       EQU 10
GE1X        EQU 52 ; edit this shit
GE1Y        EQU 41 ; this too
        
GameEnded2  DB "To continue press any key"
GE2sz       EQU 25
GE2X        EQU 45 ; this too
GE2Y        EQU 42 ; this too
...
GAMELP: 
        MOV keyDebounce, 0     ; Clear debounce flag each frame | still feeling special mofo
        CALL ParseInput
        CALL PieceGravity
        MOV AL,GameFlag
        CMP AL,1
...
SetNextPieceData    PROC    NEAR
        ...
        MOV BP, SI              ; Preserve SI which holds screen ID (0 or 4)
        
        MOV CX, 0d          ; use another register for the loop counter - waku waku
        MOV [DI], BX        ;move id of selected piece to selectedScreenPiece
        MOV AH, 0
        MOV [DI+1], AH      ;set orientation to 0
        MOV AH, 5D ; 0d origvalue apple
        MOV [DI+3], AH      ;set pieceY to 0 | orignal value was 3
        MOV AH, 04D         ;set pieceX to 4
        MOV [DI+2], AH
        
        ; Reset change piece usage flag for this screen wiggas
        CMP BP, 0
        JNZ NotLeftPlayer
        MOV canChangePieceP1, 1
        JMP DoneResetChangePiece

        NotLeftPlayer:
        MOV canChangePieceP2, 1

        DoneResetChangePiece:
        
       ; MOV canChangePiece, 1 ; this allows to change piece once per block
...
SETSCRPIECELOOP:
    ...
    JNZ SETSCRPIECELOOP
        ...
        ; MOV CL, firstPiece[BX][SI] samgyup
        ; MOV [DI], CL
        ; INC DI
        ; INC SI
        ; CMP SI, 16d
...
LeftPowerup4:;
        ...
        CMP canChangePieceP1, 0
        JE  BreakPowerup4        ; already used this turn ? skip kokodayo

        MOV SI, 0
        CALL ChangePiece

        MOV canChangePieceP1, 0    ; mark as used
...
LeftPowerup5:
        LeftPowerup5:
        CMP AH, leftPower5
        JNE @NotLeftPower5  ; Local label for short jump

        ; Check debounce
        MOV AL, 1
        XCHG AL, keyDebounce
        CMP AL, 0
        JNE @SkipLeftPower5  ; Local label for short jump

        ; Check availability
        CMP canInstantDropP1, 0
        JE @SkipLeftPower5   ; Local label for short jump

        ; Execute powerup
        MOV SI, 0
        CALL InstantDropPiece
        MOV canInstantDropP1, 0
        JMP BreakParseInput

@SkipLeftPower5:    ; Local label for skipping
        JMP BreakParseInput

@NotLeftPower5:     ; Continues to right powerup checks
        
        ;--------------------------------------------------
        ;CMP AH, leftPower5
        ;JNE RightPowerup1  ; Change to JNE (near jump) if needed
        ...
        ;CALL InstantDropPiece
        ;MOV canInstantDropP1, 0
        ;JMP BreakParseInput    ; Explicitly jump to break
...
RightPowerup4:
    ...
    CMP canChangePieceP2, 0
    JE  BreakPowerup4_Right

    MOV SI, 4
    CALL ChangePiece

    MOV canChangePieceP2, 0
BreakPowerup4_Right:
    JMP BreakParseInput
...
RightPowerup5:
        ...
        XCHG AL, keyDebounce
        CMP AL, 0
        JNE BreakParseInput    ; Skip if key was pressed last frame

        CMP canInstantDropP2, 0
        JE BreakParseInput

        MOV SI, 4
        CALL InstantDropPiece
        MOV canInstantDropP2, 0
        JMP BreakParseInput
            ;CMP AH, RightPower5
            ;JNZ BreakParseInput
...
CheckCollisionWithFrameLoop:            
        ...
        JAE outOfScreen
        CMP DX, 0       ; this too
        JL outOfScreen ;Random chatgpt shit
        CMP DX, FRAMEHEIGHT
        JAE outOfScreen 
...
;---------------------------
; Instant drop (replaces InsertTwoLines functionality)
; @param SI: screenId (0=left, 4=right)
InstantDropPiece PROC NEAR
    PUSHA
    
    ; Check if available for current piece
    CMP SI, 0
    JNE CheckRightDrop
    CMP canInstantDropP1, 0
    JE DropDone               ; If already used this piece
    MOV canInstantDropP1, 0   ; Mark as used
    JMP PerformDrop
...
PerformDrop:
    CALL GetTempPiece
    CALL DeletePiece          ; Clear current position
...
DropLoop:
    MOV BX, 0                 ; Direction = down
    CALL MovePiece            ; Try to move down
    JZ DropDone               ; Stop if collision
    JMP DropLoop              ; Continue dropping
    
DropDone:
    CALL DrawPiece            ; Redraw at final position
    POPA
    RET
InstantDropPiece ENDP
...
GenerateRandomPiece     PROC    NEAR
                        ...
                        CMP SI, 0               ; Check which player we're generating for
                        JNE ResetRightPlayerFlags
                        
                        ; Reset left player flags
                        MOV canChangePieceP1, 1  ; Allow changing piece again
                        MOV canInstantDropP1, 1  ; Allow instant drop again
                        JMP AfterFlagReset
                        
ResetRightPlayerFlags:
                        ; Reset right player flags
                        MOV canChangePieceP2, 1  ; Allow changing piece again
                        MOV canInstantDropP2, 1  ; Allow instant drop again
...
ShiftLinesUp    PROC    NEAR
                ...
                MOV DX, 5D              ;initialize dx at 0
SHIFTUPLOOPY:
                MOV CX, 0D ; orange orig is 0
SHIFTUPLOOPX:
                INC DX
...
SHIFTDOWNLOOPX:
                ...
                DEC DX  
                CMP DX, 5D              ;check if Y is = 0 banana
                JNZ SHIFTDOWNLOOPY
